...
blastGreenFiles<-file.path('work',basename(sub('\\.fasta','.green.blast.gz',faFiles)))
names(faFiles)<-names(blastFiles)<-names(blastAllFiles)<-names(blastBactFiles)<-names(blastVirginFiles)<-names(blastGreenFiles)<-sub('.fasta','',sub('_100bp_.*','',faFiles))
...
blastGreen<-mclapply(blastGreenFiles,read.table,header=FALSE,mc.cores=10,col.names=c('qName','tName','percIdent','alignLength','mismatch','gapOpens','qStart','qEnd','tStart','tEnd','eVal','bitScore'),stringsAsFactors=FALSE)
names(seqs)<-names(blasts)<-names(blastAlls)<-names(blastBacts)<-names(blastVirgin)<-names(blastGreen)<-names(faFiles)
...
blastGreenCover<-lapply(blastGreen,condenseBlast)
...
for(ii in names(blastCover)){
  ...
  results[results$set==ii,'greenCover']<-blastGreenCover[[ii]][results[results$set==ii,'name']]
...
results[is.na(results$greenCover),'greenCover']<-0
...
results$greenProp<-results$greenCover/results$nchar
...
means<-do.call(rbind,lapply(
  split(results[,c('blastProp','blastAllProp','bactProp','greenProp')],results$set),
  function(xx)apply(xx,2,mean)
))
baseProps<-do.call(rbind,mapply(
  function(xx,yy)apply(xx,2,sum)/apply(yy,2,sum),
  split(results[,c('blastCover','blastAllCover','bactCover','greenCover')],results$set),
  split(results[,c('nchar','nchar','nchar','nchar')],results$set),
  SIMPLIFY=FALSE
))
colnames(means)<-colnames(baseProps)<-c('Phage','Virus','Bacteria','16S')
